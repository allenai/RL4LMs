...
from rl4lms.envs.text_generation.warm_start import ActorCriticWarmStartMixin, MaskableActorCriticWarmStartMixin
from rl4lms.algorithms.common.maskable.distributions import MaskableCategoricalDistribution
from rl4lms.algorithms.common.maskable.logits_processor import MaskLogitsProcessorCasualLM
...
class MaskedCausalLMActorCriticPolicy(CausalLMActorCriticPolicy, MaskableActorCriticWarmStartMixin):
    def __init__(self, observation_space: DictSpace,
        ...
        super().__init__(observation_space, action_space, lr_schedule, model_name,
            optimizer_kwargs, weight_decay, use_sde, apply_model_parallel, optimizer_class,
            generation_kwargs, prompt_truncation_side, state_dict)
    ...
    def _build_model_heads(self,
        ...
        self.logits_processor = MaskLogitsProcessorCasualLM(
            self._mask_model, self.action_space, 
            self.top_mask, self._apply_model_parallel, 
            self.get_policy_first_device, self.mask_type, 
    ...
    def evaluate_actions(
        ...
        policy_outputs = self.forward_policy(obs=obs, actions=actions, action_masks=action_masks)
        value_outputs = self.forward_value(obs)
    ...
    def generate(self, tokenizer: AutoTokenizer,
        ...
        gen_output = GenerationOutputs(step_wise_logprobs,
            step_wise_actions,
            gen_tokens,
            gen_texts,
            action_masks)
