...
class LMActorOnlyPolicy(BasePolicy, ActorOnlyWarmStartMixin):
    def __init__(self, observation_space: DictSpace,
                 action_space: Discrete,
                 lr_schedule: Schedule,
                 model_name: str,
    ...
    def forward_policy(self, obs: TensorDict,
        ...
        return next_token_logits, actions, log_prob, entropy, output, past_model_kwargs
    ...
    def evaluate_actions(self, obs: torch.Tensor,
        ...
        next_token_logits, _, log_prob, entropy, output, _ = self.forward_policy(obs=obs,
                                                                                 actions=actions)

        return next_token_logits, log_prob, entropy, output
...
class Seq2SeqLMActorOnlyPolicy(LMActorOnlyPolicy):
    def _build_model_heads(self,
                           model_name: str):
        self._policy_model = AutoModelForSeq2SeqLM.from_pretrained(
            model_name)
    ...
    def forward_policy(self, obs: TensorDict,
        ...
        model_kwargs["decoder_attention_mask"] = torch.cat(
            (decoder_attn_mask, torch.ones(batch_size, 1).to(decoder_attn_mask.device)), dim=-1)
        return next_token_logits, actions, log_prob, entropy, outputs, model_kwargs
