...
class OffPolicyTrainer(TrainerWarmStartMixin):
    """
    A generic trainer for training LMs with offpolicy algorithms like online decision transformer
    """
    ...
    def __init__(self,
                 ...
                 reward_config: Dict[str, Any],
                 env_config: Dict[str, Any],
                 off_policy_alg_config: Dict[str, Any],
                 train_eval_config: Dict[str, Any],
                 tracker: Tracker = None,
                 experiment_name: str = ''
                 ):
        ...
        self._reward_config = reward_config
        self._env_config = env_config
        self._off_policy_alg_config = off_policy_alg_config
        self._train_eval_config = train_eval_config
        self._tracker = tracker
        self._experiment_name = experiment_name
        self._setup()
    ...
    def _setup(self):
        ...
        self._env = build_env(self._env_config, self._reward_fn,
                              self._tokenizer, self._samples_by_split["train"])
        self._alg = build_alg(self._off_policy_alg_config,
                              self._env, self._tracker,
                              self._policy_state_dict,
                              self._alg_state_dict)
        ...
        self._max_prompt_length = self._env_config["args"]["max_prompt_length"]
        self._eval_batch_size = self._train_eval_config["eval_batch_size"]
        self._control_token = self._train_eval_config["control_token"]
        self._n_iters = int(self._train_eval_config["n_iters"])
        self._n_steps_per_iter = self._env.num_envs * \
            self._off_policy_alg_config["args"]["n_steps"]
    ...
    def _evaluate_on_datapools(self, epoch: int,
        ...
        for split in splits:
            evaluate_on_samples(policy=self._alg.policy,
                                ...
                                split_name=split,
                                tracker=self._tracker,
                                dt_control_token=self._control_token
                                )
