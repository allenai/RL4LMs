...
from rl4lms.envs.text_generation.metric import CIDERMetric, MeteorMetric, BERTScoreMetric, BLEUMetric, SpiceMetric, ParentToTTo, RougeLMax, TERMetric, chrFmetric
import numpy as np
...
class BatchedRewardFunction(ABC):
    ...
    def __call__(self, prompt_texts: List[str],
                 ...
                 dones: List[bool],
                 meta_info: Dict[str, Any] = None) -> List[float]:
        """
...
class CommonGenPenaltyShapingFunction(RewardFunction):
    def __call__(self, current_observation: Observation,
        ...
        if done:
            ...
            concept_n_grams = prompt_text.split(
                prefix)[1][:-1]

            if concept_n_grams.lower() in next_observation.context_text.lower() or \
                prefix in next_observation.context_text.lower() or \
                "generate" in next_observation.context_text.lower() or \
               "sentence" in next_observation.context_text.lower():
                penalty_score = -1
            else:
                penalty_score = 0
    ...
    def __call__(self, prompt_texts: List[str],
        ...
        for done, prompt_text, gen_text in zip(dones, prompt_texts, gen_texts):
            if done:
                ...
                if concept_n_grams.lower() in gen_text.lower() or \
                    prefix in gen_text.lower() or \
                    "generate" in gen_text.lower() or \
                        "sentence" in gen_text.lower():
                    penalty_score = -1
                else:
                    penalty_score = 0
...
class MeteorRewardFunction(RewardFunction):
    def __init__(self, shaping_fn: str = None) -> None:
        ...
        self._shaping_fn = RewardFunctionRegistry.get(
            shaping_fn, {}) if shaping_fn is not None else shaping_fn
...
class SpiderRewardFunction(BatchedRewardFunction):
    def __init__(self, spice_coeff: float,
                 cider_coeff: float,
                 shaping_fn: str = None) -> None:
        """
        Spice + Cider
        """
        super().__init__()
    ...
    def __call__(self, prompt_texts: List[str],
        ...
        if len(indices_with_done) > 0:
            spice_scores = self._spice_metric.compute(prompts, gens, refs)[
                "lexical/spice"][0]
            cider_scores = self._cider_metric.compute(
                prompts, gens, refs)["lexical/cider"][0]
            total_scores = self._spice_coeff * \
                np.array(spice_scores) + self._cider_coeff * \
                np.array(cider_scores)

            if self._shaping_fn is not None:
                aux_scores = self._shaping_fn(
                    prompt_texts, gen_texts, ref_texts, dones)
            else:
    ...
    def __call__(self, current_observation: Observation,
        ...
        if done:
            ...
            with torch.no_grad():
                ...
                outputs = self._metric_model(input_ids=encoded.input_ids.to(self._device),
    ...
    def __call__(self, current_observation: Observation,
        ...
        if done:
            ...
            scores = self._metric.compute(
                None, generated_texts, None, meta_infos)
...
class RougeLMaxRewardFunction(RewardFunction):
    ...
    def __call__(self, current_observation: Observation,
        ...
        if done:
            ...
            meta_infos = [meta_info]
            scores = self._metric.compute(
                None, predicted, references, meta_infos)
...
if __name__ == "__main__":
    ...
    observation = Observation(None, None, None, None,
                              None, predictions, references, None, None, None, None)
