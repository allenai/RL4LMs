from transformers import LogitsProcessor, AutoModelForCausalLM, AutoModelForSeq2SeqLM
...
class MaskLogitsProcessorCasualLM(LogitsProcessor):
    def __init__(self, mask_model, action_space, top_mask, apply_model_parallel, get_policy_first_device, mask_type, min_tokens_to_keep):
    ...
    def _prepare_inputs_for_model(self, model: Union[AutoModelForCausalLM, AutoModelForSeq2SeqLM],
        ...
        if self._apply_model_parallel:
            ...
            model_inputs = {key: value.to(self.get_policy_first_device()) if isinstance(
                value, torch.Tensor) else value for key, value in model_inputs.items()}
        return model_inputs

    def _get_action_masks(self, input_ids: torch.Tensor, scores: torch.FloatTensor, model_inputs: dict = None) -> torch.Tensor:
        ...
        if model_inputs is not None:
            model_inputs = self._prepare_inputs_for_model(self.mask_model,
                                                          input_ids,
                                                          self.past_model_kwargs)
        if self.mask_type == 'learned_top_k':
            ...
            self.past_model_kwargs = self.mask_model._update_model_kwargs_for_generation(
                output, self.past_model_kwargs, is_encoder_decoder=self.mask_model.config.is_encoder_decoder
            )
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor, model_inputs: dict = None) -> torch.FloatTensor:
        self.device = scores.device
        action_masks = self._get_action_masks(input_ids, scores, model_inputs)
        action_masks = torch.as_tensor(
            action_masks, dtype=torch.bool, device=self.device).reshape(scores.shape)
